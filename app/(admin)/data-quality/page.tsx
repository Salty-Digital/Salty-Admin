...
export default async function DataQualityPage() {
  ...
  const enrichment = [
    { label: 'Theatre — cast', total: theater.length, done: theater.filter((t) => cast.has(t.id)).length },
    { label: 'Concerts — setlist (past)', total: pastConcertish.length, done: pastConcertish.filter((t) => setlist.has(t.id)).length },
    { label: 'Sports — result (past)', total: pastSports.length, done: pastSports.filter((t) => sportsFinal.has(t.id)).length },
  ]
  ...
  const confBars = [
    { label: 'High (≥ 80%)', value: conf.high, color: '#3E8A5A' },
    { label: 'Medium (50–80%)', value: conf.medium, color: '#C8A96E' },
    { label: 'Low (< 50%)', value: conf.low, color: '#E8581A' },
    { label: 'Unknown', value: conf.unknown, color: '#9A8F82' },
  ]
  ...
  return (
    <div className="p-7 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <QualityCard label="Categorised" pct={pct(categorised, total)} sub={`${(tickets.length - categorised).toLocaleString()} still “other”`} />
      ...
      <Panel icon={Sparkles} title="Enrichment completeness" hint="of the events that should have it">
        {enrichment.map((e) => (
          <Bar key={e.label} label={e.label} value={e.done} total={e.total} color="#7B44A8"
      ...
      <Panel icon={ShieldCheck} title="Import confidence" hint="distribution across all tickets">
        {confBars.map((b) => (
          <Bar key={b.label} label={b.label} value={b.value} total={tickets.length} color={b.color}
...
function QualityCard({ label, pct, raw, sub }: { label: string; pct?: number; raw?: number; sub: string }) {
...
function Bar({ label, value, total, color, right }: { label: string; value: number; total: number; color: string; right: string }) {
  ...
  return (
    <div className="flex items-center gap-4 border-b border-salty-border px-5 py-3 last:border-0">
